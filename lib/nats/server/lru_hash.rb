# LRU based Hash
# Based on previous work by Robert Klemme on his blog
# http://blog.rubybestpractices.com/posts/rklemme/013-LRU-Explanations.html
...
class LRUHash
  ...
  def to_s
    s = nil
    each_pair {|k, v| (s ? (s << ', ') : s = "lru(#{@max_size}/#{size}){") << k.to_s << '=>' << v.to_s}
    s ? (s << '}') : "lru(#{@max_size}/#{size}){"
  end
...
if __FILE__ == $0
  ...
  lru = LRUHash.new(CACHE)
  hsh = {}
  
  print "1,000,000 times LRUHash: "
  start = Time.now
  1000_000.times do |i|
    lru[i] = i
  end
  puts(Time.now - start)
  puts "LRUHash size: #{lru.size}"
  
  
  print "1,000,000 times Hash: "
  start = Time.now
  1000_000.times do |i|
    hsh = {} if hsh.size > CACHE
    hsh[i] = i
  end
  puts(Time.now - start)
  puts "hash size: #{hsh.size}"
  
  
  puts "Unfortunately the plain old hash with a check for size every time is 
  still an order of magnitude faster then the LRUHash :( of well nice try"
end